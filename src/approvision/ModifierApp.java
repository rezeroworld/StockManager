...
public class ModifierApp implements Initializable {
    ...
    public void modifier(javafx.event.ActionEvent event) throws IOException {
        int newStock = Integer.valueOf(quant.getText()) - GererApprovision.selectedItem.getQte_a();
        ...
        try{
            ...
            if(result.get() == ButtonType.OK) {
                PreparedStatement create = Menu.con.prepareStatement("update detail_app set qte_a = ? where (n_bon_a = ? and n_article = ?)");
                ...
                create.setString(3, GererApprovision.selectedItem.getDate_app());
                create.executeUpdate();

                create = Menu.con.prepareStatement("update stock set qte_a = ? where (n_article = ? and date = ?)");
                create.setInt(1, Integer.valueOf(quant.getText()));
                create.setString(2, article.getText());
                create.setString(3, GererApprovision.selectedItem.getDate_app());
                create.executeUpdate();

                GererApprovision.selectedItem = null;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        numBon.setText(GererApprovision.selectedItem.getNumBon());
        article.setText(GererApprovision.selectedItem.getNum());
        quant.setText(String.valueOf(GererApprovision.selectedItem.getQte_a()));
    }
}
