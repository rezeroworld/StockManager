...
public class gererArticleController implements Initializable {
    ...
    public void remplir_list() throws SQLException {
        ...
        try {
            PreparedStatement create = Main.con.prepareStatement("select distinct n_article from article");
            ...
            for(int i = 0; i < numeros.size(); i++) {
                PreparedStatement create2 = Main.con.prepareStatement("select article.n_article, libelle, prix_unit, stock_min, stock from article, stock where article.n_article = ? and article.n_article = stock.n_article order by date desc");
    ...
    public void supprimer_article() throws IOException, SQLException {
        ...
        if(result.get() == ButtonType.OK) {
            ...
            PreparedStatement create = Main.con.prepareStatement("delete from article where n_article = ?");
