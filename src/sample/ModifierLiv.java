        ...
        try{
            if((cs >= newStockDiff) && test) {
                ...
                if (result.get() == ButtonType.OK) {
                    PreparedStatement create = Main.con.prepareStatement("update detail_liv set qte_l = ? where (n_bon_l = ? and n_article = ?)");
                    ...
                    create = Main.con.prepareStatement("update stock set qte_l = ? where (n_article = ? and date = ?)");
                    create.setInt(1, Integer.valueOf(quant.getText()));
                    create.setString(2, article.getText());
                    create.setString(3, GererLiv.selectedItem.getDate_liv());
                    create.executeUpdate();
